---
title: "Obtain variance for relative scale difference-in-difference"
...
$$\text{RDID} = 1 - \bigg( \frac{(E[Y|X=1, T=1]-E[Y|X=1, T=0])}{ E[Y|X=0, T=1] - E[Y|X=0, T=0]}  \bigg) \times 100$$
  
Using the coefficients from the model above, the transformation function is:

$$ F(\beta) = \bigg(1 - \frac{e^{\beta_0 + \beta_1 + \beta_2 + \beta_3} - e^{\beta_0 + \beta_1}}{ e^{\beta_0 + \beta_2 } - e^{\beta_0}}\bigg) \times 100    $$
...
# relative scale DID x 100%
(1-(((158/447832) -(117/447832))  / ((113/255318) -(56/255318)))) * 100
...
# Use Delta Method to get variance for relative-scale difference-in-difference

## Step 1: Get partial derivatives of $F(\beta)$ with respect to each parameter 

$$\frac{dF}{d\beta_0} = 0$$

$$\frac{dF}{d\beta_1} = \frac{e^{\beta_2}(e^{\beta_2 + \beta_3}-1)}{(e^{\beta_2} - 1)^2} $$

$$\frac{dF}{d\beta_2} = \frac{(e^{\beta_3}-1)e^{\beta_1+ \beta_2}}{(e^{\beta_2}-1)^2}$$

$$\frac{dF}{d\beta_3} = \frac{e^{\beta_1+\beta_2+\beta_3}}{1-e^{\beta_2}} $$
...
rr = (exp(b0+b1+b2+b3) - exp(b0+b1))/(exp(b0+b2) - exp(b0))
rdid = (1-rr) * 100 
